...
def ho(datasetId = None, network = None, nGPU = None, subTorun=None, AugumentEnable=True, centerLossEnable=False):
    ...
    config['AugumentEnable'] = AugumentEnable
    # Data load options:
    ...
    config['modelTrainArguments'] = {'stopCondi':  {'c': {'Or': {'c1': {'MaxEpoch': {'maxEpochs': 1500, 'varName' : 'epoch'}},
          ...
          'bestVarToCheck': 'valInacc', 'continueAfterEarlystop':True,'lr': 1e-3, 'epsilon':0.02}
   ...
   #%% Find all the subjects to run 
    ...
    for iSub, sub in enumerate(subs):
        ...
        if config['network'] == 'FBNASNet':
            ...
            start = time.time()
            if datasetId == 1:
                opt_choice, cali_bn_acc = nas_phase(supernet,trainData, valData, classes=2,supernet_path=os.path.join(outPathSub,'supernet.pth'))
            if datasetId == 0:   
                opt_choice, cali_bn_acc = nas_phase(supernet,trainData, valData, classes=4,supernet_path=os.path.join(outPathSub,'supernet.pth'))
            SearchTime_list[str(sub)] = str(time.time()-start)
            np.save(outPathSub+'/cali_bn_acc.npy', np.array(cali_bn_acc))
            dictToCsv(os.path.join(outPathSub,'opt_choice.csv'), opt_choice)
            net = network(**config['modelArguments'],opt_choice = opt_choice)
            net_dict = net.state_dict()
            pretext_model = torch.load(os.path.join(outPathSub,'supernet.pth'))
            # print(pretext_model)
        ...
        model.train(trainData, valData, testData, **config['modelTrainArguments'],centerLossEnable=config['centerLossEnable'], AugumentEnable=config['AugumentEnable'], init_centers=init_centers)
        torch.save(net, os.path.join(outPathSub,'net.pth'))
...
if __name__ == '__main__':
    ...
    if count >3:
        ...
        subTorun = None
    ho(datasetId=0, network='FBNASNet', nGPU='0', subTorun=subTorun, AugumentEnable=False, centerLossEnable=False)
