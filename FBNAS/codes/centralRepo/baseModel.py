...
class baseModel():
    ...
    def train(
        ...
        if centerLossEnable:
            # CenterLoss
            loss_weight = 1
            centerloss = CenterLoss(num_classes=4, feat_dim=2304, init_centers=init_centers).to(self.device)
            # centerloss = CenterLoss(num_classes=4, feat_dim=1152, init_centers=init_centers).to(self.device)
            # optimzer4center
    ...
    def _trainOE(
        ...
        while not doStop:
            ...
            loss.append(self.trainOneEpoch(trainData, lossFn, self.optimizer, sampler = sampler, epsilon = epsilon, AugumentEnable = AugumentEnable, centerLossEnable=centerLossEnable,
    ...
    def trainOneEpoch(self, trainData, lossFn, optimizer, sampler = None, epsilon= None, AugumentEnable = False, centerLossEnable=False,
        ...
        for d in dataLoader:
            with torch.enable_grad():                                                                      
                if centerLossEnable:
                    ...
                    data = d['data'].unsqueeze(1).to(self.device)
                    label = d['label'].type(torch.LongTensor).to(self.device)
                    if AugumentEnable:
                        data, label = rta(data, label)
                    
                    # forward pass
                    output,feature = self.net(data)

                    # calculate loss
                    closs = centerloss(label, feature)

                    loss = lossFn(output, label)
                    loss = loss / data.shape[0]

                    # loss = loss + Lambda*closs
                    # loss.backward()
