#!/bin/bash
# Submit nine DARTS search runs, one subject each.  Same quota-aware loop as
# submit_fbnas.sh; only the job script and the seed differ.
set -u
...
SEED="${SEED:-20250901}"
AGENDA="$RUNS/sh_log/darts_agenda.txt"
: > "$AGENDA"
...
submit_one() {
  ...
  for entry in "GPUFEE04 20 $r04" "GPUFEE05 2 $r05" "GPUFEE02 2 $r02"; do
    ...
    out=$(SUB=$sub SEED="$SEED" sbatch -p "$P" bin/run_darts_job.sh 2>&1)
